...
int main(int argc, char** argv)
{
	...
	//Setup buffer for initial connection request(CL_CON_REQ)
	...
	//If the server doesnt answer, contact him for three times
	for (tries = 0; tries < 3; tries++)
	{
		...
		else
		{
			...
			//If the server did answer, save the new Port
	...
	if (tries == 3)
	{
		printf("Server doesnt answer... Shutting down.\n");
		exit(1);
	}
	...
	while (1)
	{
		...
		FD_SET(fd, &fds);
		FD_SET(0, &fds);
		//select between input from socket or stdin
		...
		//if theres a new server msg available...
		if (FD_ISSET(fd, &fds))
		{
			...
			case SV_DISC_AMSG:
			{
				...
				//server message because of a newly joined user
			case SV_CON_AMSG:
			{
				uint16_t userNameLength;
		...
		if (FD_ISSET(0, &fds))
		{
			...
			{
				// otherwise send usermessage to server
